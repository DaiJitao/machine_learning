...
# 输入的input为，序列长度seq_len=5, 即x1,...,x5
# 每次取的minibatch大小，batch_size=3, 数据向量维数=10（仍然为x的维度）。每次运行时取3个含有5个字的句子（且句子中每个字的维度为10进行运行）
...
print(output.shape)
# 输出：torch.Size([3, 5, 20]) torch.Size([2, 3, 20]) torch.Size([2, 3, 20])
